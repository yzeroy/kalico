...
def plot_system(data):
    ...
    ax1.plot_date(
        times, sysloads, "-", label="system load", color="cyan", alpha=0.8
    )
    ax1.plot_date(
        times, cputimes, "-", label="process time", color="red", alpha=0.8
    )
    ax2 = ax1.twinx()
...
def main():
    ...
    opts.add_option(
        "-s", "--system", action="store_true", help="graph system load"
    )
    opts.add_option(
        "-o",
        "--output",
