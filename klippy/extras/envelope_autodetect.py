...
class EnvelopeAutoDetect:
    def __init__(self, config):
        ...
        self.gcode.register_command(
            "DETECT_RACKING", self.cmd_DETECT_RACKING, desc="detect racking"
        )
    ...
    def home_axes_normally(self, axes: list[str]):
        self.gcode.run_script_from_command(
            f"G28 {' '.join([axis.upper() for axis in axes])}"
        )
    ...
    def cmd_DETECT_RACKING(self, gcmd):
        ...
        gcmd.respond_info("racking raw data:")
    ...
    def do_autodetect_travel_for_axis(self, axis, update_limits=True):
        ...
        for _ in range(self.iterations):
            self.do_single_home(axis, direction)
            length = self.do_single_home(axis, not direction)
            distances.append(length)
        if homing_positive_dir:  # make sure we end at right side
            self.do_single_home(axis, True)
    ...
    def do_single_home(
        ...
        self.kin.set_axis_limits(
            axis_index,
            (min_pos[axis_index] - buffer, max_pos[axis_index] + buffer),
        )
        ...
        gcode_move = self.printer.lookup_object("gcode_move")
        gcode_move.reset_last_position()
        self.kin.set_axis_limits(axis_index, old_limits)
