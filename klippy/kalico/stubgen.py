...
def build_printer_status_type(printer: klippy.printer.Printer):
    ...
    for obj_name, obj in printer.lookup_objects():
        ...
        printer_status[obj_name] = typing.Annotated[
            infer_typeddict(
                pascal_case(f"{obj_name}_status"),
                obj_status,
            ),
            "Inferred type stub, may be incomplete",
        ]
...
def qualified_name(typ):
    ...
    if typ.__module__ == "collections.abc":
        return f"collections.abc.{typ.__name__}"
...
def print_type(typ, file=sys.stdout):
    if is_typeddict(typ):
        ...
        if all(key.isidentifier() for key in fields.keys()):
            print(
                f"class {qualified_name(typ)}(typing.TypedDict):",
