...
use_clang = sysconfig.get_config_var("CC").startswith("clang")
...
C_INCLUDES = "\n".join(f'#include "{f.name}"' for f in HEADER_FILES)
...
defs_pyhelper = """
    void set_python_logging_callback(void (*callback)(const char *));
...
defs_hubctrl = """
    ...
    int hubctrl_set_power(int hub, int port, int value);
"""
...
def compile():
    with tempfile.TemporaryDirectory() as tmpdir:
        lib_so = ffi_builder.compile(tmpdir=tmpdir)
        shutil.copy(lib_so, dir)


def needs_rebuild():
    """
    Check if the c-code needs to be rebuilt
    """
    spec = importlib.util.find_spec("._ffi", "klippy.chelper")
    if not spec:
        return True

    ffi_so = pathlib.Path(spec.origin)

    if (
        max(
            f.stat().st_mtime
            for f in ([pathlib.Path(__file__)] + SOURCE_FILES + HEADER_FILES)
        )
        > ffi_so.stat().st_mtime
    ):
        return True
